...
//             external service. Real mode calls any OpenAI-compatible
...
// different persona. The mock reviews are varied enough to demo —
// ratings spread across the 1-10 range, plausible feedback text.
const MOCK_TEMPLATES = {
  production: {
    system: 'You are a music production critic specializing in audio quality, mix, and mastering.',
    getReview(genre, versionType) {
      const seed = crypto.createHash('md5').update(`${genre}:${versionType}:production`).digest();
      const solo = 3 + (seed[0] % 7);
      const vocal = 4 + (seed[1] % 6);
      const energies = ['lower', 'same', 'higher'];
      const tempos = ['dragging', 'locked', 'rushing'];
      return {
        solo_intensity: solo,
        vocal_quality: vocal,
  ...
  performance: {
    ...
    getReview(genre, versionType) {
      const seed = crypto.createHash('md5').update(`${genre}:${versionType}:performance`).digest();
      ...
      return {
        ...
        tempo_feel: tempos[seed[3] % 3],
        mood_tags: pickMoods(seed, genre),
        notes: mockPerformanceNotes(genre, versionType, solo, vocal)
      };
    }
  },
  market: {
    ...
    getReview(genre, versionType) {
      const seed = crypto.createHash('md5').update(`${genre}:${versionType}:market`).digest();
...
const ALL_MOODS = ['bluesy', 'raw', 'euphoric', 'melancholic', 'aggressive', 'dreamy', 'groovy', 'intimate', 'cinematic', 'nostalgic'];

function pickMoods(seed, genre) {
...
function mockProductionNotes(genre, versionType, solo, vocal) {
  const notes = [];
...
function mockPerformanceNotes(genre, versionType, solo, vocal) {
  const notes = [];
...
function mockMarketNotes(genre, versionType) {
...
function mockPlacementBrief(genre, versionType) {
  const genreVenues = {
    'rock': [
  ...
  return {
    ...
    audience_summary: `The ${genre} ${versionType} market is strongest among 25-40 year old listeners who value authenticity and variety. ` +
...
function createLlmAdapter({ apiUrl, apiKey, model = 'gpt-4o-mini', requestTimeoutMs = DEFAULT_TIMEOUT } = {}) {
  ...
  return {
    ...
    async complete({ system, user, temperature = 0.7, maxTokens = 2000, agentName, genre, versionType } = {}) {
      if (useMock) {
      ...
      try {
        ...
        console.warn(`[llm] API call failed, falling back to mock: ${err.message}`);
        const template = MOCK_TEMPLATES[agentName] || MOCK_TEMPLATES.production;
        const review = template.getReview(genre || 'rock', versionType || 'live');
        return {
          text: JSON.stringify(review),
...
module.exports = { createLlmAdapter, MOCK_TEMPLATES };
