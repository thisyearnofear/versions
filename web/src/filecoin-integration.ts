...
import type { 
    FilecoinStorageInfo, 
    FilecoinUploadRequest,
    CreatorPaymentRequest,
    PaymentRail,
    NetworkStatus,
    AudioMetadata,
    ApiResponse
...
// CLEAN: Types for Filecoin integration
interface ProgressCallback {
    (message: string, progress: number): void;
}

interface StorageUploadResult extends FilecoinStorageInfo {
    cdn_url: string;
...
interface CreatorEarnings {
    total_earnings_usdfc: string;
    total_earnings_usd: string;
    active_rails: number;
    version_earnings: VersionEarning[];
    last_updated: string;
}

interface VersionEarning {
    rail_id: string;
    version_id: string;
    title: string;
    earnings_usdfc: string;
    earnings_usd: string;
    play_count: number;
    last_payment: string;
}
...
// MODULAR: Types for Synapse SDK integration
interface SynapseSDK {
    createStorage(options: any): Promise<any>;
    payments: {
        walletBalance(token: any): Promise<bigint>;
        allowance(address: string, token: any): Promise<bigint>;
        approve(address: string, amount: bigint, token: any): Promise<any>;
        deposit(amount: bigint): Promise<any>;
        approveService(address: string, epochRate: bigint, lockup: bigint, maxPeriod: bigint): Promise<any>;
        createRail(token: any, from: string, to: string, validator: null, commission: number, serviceFeeRecipient: null): Promise<string>;
        modifyRailPayment(railId: string, recurringRate: bigint, oneTimeAmount: bigint): Promise<void>;
        getRailsForPayeeAndToken(address: string, token: any): Promise<PaymentRailData[]>;
        getRailSettlements(railId: string): Promise<SettlementData[]>;
...
class FilecoinIntegration {
    private synapse: SynapseSDK | null;
    private isInitialized: boolean;
    private network: string;
    // PERFORMANT: Cache for storage info
    private storageCache: Map<string, FilecoinStorageInfo>;
    ...
    async loadSynapseSDK(): Promise<boolean> {
        if (!this.synapse) {
            try {
                ...
                const { Synapse } = await (new Function('return import("https://esm.sh/@filoz/synapse-sdk")')()) as any;
                ...
                console.warn('Filecoin SDK initialization failed:', (error as Error).message);
                return false;
            }
        }
        return true;
    ...
    async uploadVersionGlobal(audioFile: File, metadata: AudioMetadata, progressCallback?: ProgressCallback): Promise<StorageUploadResult> {
        try {
            ...
            const storageInfo: StorageUploadResult = {
            ...
            throw new Error(`Global upload failed: ${(error as Error).message}`);
        }
    }

    // MODULAR: Ensure storage payment is available
    private async ensureStoragePayment(fileSize: number, progressCallback?: ProgressCallback): Promise<void> {
        try {
            if (!this.synapse) {
            ...
            const { TOKENS, TIME_CONSTANTS } = await (new Function('return import("https://esm.sh/@filoz/synapse-sdk")')()) as any;
    ...
    private calculateStorageCost(fileSize: number): bigint {
    ...
    async streamFromGlobal(pieceCid: string): Promise<Blob> {
        try {
    ...
    async supportCreator(creatorAddress: string, usdAmount: number, message: string = ''): Promise<CreatorPaymentResult> {
        try {
            if (!this.isAvailable()) {
                await this.loadSynapseSDK();
            }
            ...
            // Import TOKENS from Synapse SDK
            const { TOKENS } = await (new Function('return import("https://esm.sh/@filoz/synapse-sdk")')()) as any;
            ...
            try {
                const paymentRequest: CreatorPaymentRequest = {
    ...
    async getStorageInfo(fileId: string): Promise<FilecoinStorageInfo | null> {
        ...
        try {
            ...
            const data: ApiResponse<FilecoinStorageInfo> = await response.json();
