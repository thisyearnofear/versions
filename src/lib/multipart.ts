...
function readLine(buf: Buffer, start: number): { line: Buffer; next: number } {
...
function parseContentDisposition(headerBuf: Buffer): { name: string | null; filename: string | null } {
...
export interface MultipartCallbacks {
  ...
  onFile: (name: string, filename: string, contentType: string, data: Buffer) => void;
}
...
export function parseMultipart({
  ...
  contentType: string;
  body: Buffer;
  onField: MultipartCallbacks['onField'];
  onFile: MultipartCallbacks['onFile'];
}): void {
  ...
  while (cursor < body.length) {
    ...
    const disposition = parseContentDisposition(
      Buffer.from(headers['content-disposition'] || ''),
    );
